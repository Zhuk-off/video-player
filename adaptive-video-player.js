...
function getScreenOrientation() {
  ...
  if (window.screen && window.screen.orientation) {
    ...
    return orientation.includes('portrait') ? ORIENTATION.PORTRAIT : ORIENTATION.LANDSCAPE;
  } 
...
function selectVideoSource(videoElementId = 'local_video') {
  ...
  if (deviceType === DEVICE_TYPES.MOBILE) {
    if (orientation === ORIENTATION.LANDSCAPE) {
      sourceElement = createVideoSource(VIDEO_SOURCES.MOBILE_LANDSCAPE, 'mobile-landscape-video');
      sourceDescription = 'мобильное горизонтальное видео';
    } else {
      sourceElement = createVideoSource(VIDEO_SOURCES.MOBILE_PORTRAIT, 'mobile-portrait-video');
  ...
  const fallbackText = document.createTextNode('Your browser does not support videos.');
  ...
  video.addEventListener('loadedmetadata', function() {
    ...
    // Если видео воспроизводилось до смены источника, продолжаем воспроизведение
    if (wasPlaying) {
...
function initAdaptiveVideoPlayer(videoElementId = 'local_video') {
  ...
  document.addEventListener('DOMContentLoaded', () => selectVideoSource(videoElementId));
  ...
  if (window.screen && window.screen.orientation) {
    window.screen.orientation.addEventListener('change', () => selectVideoSource(videoElementId));
  } else {
    window.addEventListener('orientationchange', () => selectVideoSource(videoElementId));
  }
}
